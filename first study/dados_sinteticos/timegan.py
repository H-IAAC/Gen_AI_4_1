...
num_samples_per_class = 1000  # Número de amostras sintéticas para gerar por classe
...
for class_label, X_class_data in class_data.items():
    ...
    embeddings = timegan.get_embeddings(X_class_data)
...
for class_label, synthetic_data in synthetic_data_by_class.items():
    synthetic_data_cpu = synthetic_data.cpu().numpy()  # Mover para CPU antes de converter para numpy
    print(f"Class {class_label} synthetic data shape: {synthetic_data_cpu.shape}")
...
# Loop through each class's synthetic data
for class_label, synthetic_data in synthetic_data_by_class.items():
    synthetic_data_cpu = synthetic_data.cpu().numpy()  # Move to CPU and convert to NumPy
    
    # Reshape the data from (100, 60, 6) to (100, 360)
    reshaped_data = synthetic_data_cpu.reshape(synthetic_data_cpu.shape[0], -1)
    
    # Create a DataFrame
    df = pd.DataFrame(reshaped_data)
    df['class_label'] = class_label  # Add class label as a column

    directory_path = f"{dataset}/{transform}"

    os.makedirs(directory_path, exist_ok=True)

    csv_filename = f"{directory_path}/synthetic_data_class_{class_label}.csv"

    df.to_csv(csv_filename, index=False)

    print(f"Saved synthetic data for class {class_label} to {csv_filename}")
